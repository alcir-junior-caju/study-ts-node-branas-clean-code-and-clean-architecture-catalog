...
test("Deve retornar os produtos", async function () {
  ...
  const output = await httpClient.get("http://localhost:3013/products");
  expect(output).toHaveLength(3);
});

test("Deve retornar o produto 1", async function () {
  const httpClient = new AxiosAdapter();
  const output = await httpClient.get("http://localhost:3013/products/1");
...
test("Deve retornar o produto 4", async function () {
  const httpClient = new AxiosAdapter();
  expect(() =>
    httpClient.get("http://localhost:3013/products/4")
